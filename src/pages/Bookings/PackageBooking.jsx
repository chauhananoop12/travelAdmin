...
import { busBooking, packageBooking } from 'api/booking';
...
export default function PackageBooking() {
    ...
    const allCategories = async () => {
        const resp = await packageBooking()
        console.log(resp)
    ...
    const dataList = {
        ...
        rows: categoryList.map((role, index) => {
            return {
                ...
                siteSeeing: role?.packageDetails?.siteSeeing?.map((items) => items),
                activities: role?.packageDetails?.activities?.map((items) => items),
                dayCount: role?.dayCount,
